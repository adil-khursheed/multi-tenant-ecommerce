...
function buildActiveFilters(params: FilterParams): FilterParam[] {
  ...
  if (params.category)
    filters.push({ key: "category", label: `Category: ${params.category}` });
  if (params.priceRange)
    filters.push({ key: "priceRange", label: `Price: ${params.priceRange}` });
  if (params.size) filters.push({ key: "size", label: `Size: ${params.size}` });
  if (params.color)
    filters.push({ key: "color", label: `Color: ${params.color}` });
  if (params.brand)
    filters.push({ key: "brand", label: `Brand: ${params.brand}` });
  if (params.rating)
    filters.push({ key: "rating", label: `Rating: ${params.rating} & Up` });
  if (params.occasion)
    filters.push({ key: "occasion", label: `Occasion: ${params.occasion}` });
  if (params.material)
    filters.push({ key: "material", label: `Material: ${params.material}` });
  return filters;
}
...
export default function ShopScreen() {
  ...
  const products = useMemo<ShopProduct[]>(
    () =>
      (productsQuery.data?.pages.flatMap((page) => page.products.docs) ??
        []) as ShopProduct[],
    [productsQuery.data],
  );
  ...
  const currentSortTitle = useMemo(() => {
    ...
    return (
      sorting.find((s) => s.slug === sortSlug)?.title ??
      sorting[0]?.title ??
      "Sort"
    );
  }, [filterParams.sort]);
