...
test.describe("Frontend", () => {
  ...
  test("should retain cart content on hard refresh", async ({ page }) => {
    ...
    const cartCount = page
      .locator('button[data-slot="sheet-trigger"] span')
      .last();
  ...
  test("Admins can update and view prices on products", async ({ page }) => {
    ...
    const productDetailsButton = page.getByRole("button", {
      name: "Product Details",
    });
    await productDetailsButton.click();

    const priceInput = page.locator(
      'input.formattedPriceInput[placeholder="0.00"]',
    );
  ...
  test("Admins can update and view prices on variants", async ({ page }) => {
    ...
    const variantPriceInput = page
      .locator('input.formattedPriceInput[placeholder="0.00"]')
      .first();
  ...
  test("Admins can create new products with new variants", async ({ page }) => {
    ...
    const chooseFromExistingButton = page.getByRole("button", {
      name: "Choose from existing",
    });
    await chooseFromExistingButton.click();
    const firstFileButton = page
      .locator("button.default-cell__first-cell")
      .first();
    ...
    const newProductCard = page
      .locator(`a[href="/products/new-product-with-variants"]`)
      .first();
  ...
  test("Admins can view transactions and orders", async ({ page }) => {
    ...
    const product = page.locator("div.rs__control", {
      hasText: "Test Product",
    });
    await expect(product).toBeVisible();

    await page.goto(`${baseURL}/admin/collections/transactions`);
    const transactionRows = await page
      .locator("div.table table tbody tr")
      .count();
  ...
  test("should disable add to cart when product has no inventory", async ({
    page,
  }) => {
  ...
  async function addToCartAndConfirm(
    ...
    const productCard = page
      .locator(`a[href="/products/${productSlug}"]`)
      .first();
    await productCard.waitFor({ state: "visible" });
  ...
  async function removeFromCartAndConfirm(page: Page) {
    const reduceQuantityButton = page.getByRole("button", {
      name: "Reduce item quantity",
    });
  ...
  async function checkout(
    ...
    if (guestEmail) {
      ...
      const continueGuestBtn = page.getByRole("button", {
